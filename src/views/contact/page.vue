<template>
  <div>
    ...
    <div v-if="!store.is_loading">
      <div class="pt-2 pl-6 uppercase font-semibold text-center"> {{ store.contact_data?.label }} </div>
      <div class="flex flex-row justify-center items-start mx-auto space-x-3">
        <div class="card w-96 bg-base-100 dark:bg-gray-800 shadow-xl">
          <figure class="px-10 pt-10">
            <img :src="store.contact_data?.icon" alt="Shoes" class="rounded-xl" />
          </figure>
          <div class="card-body items-center text-center">
            <form class="space-y-2" @submit="store.updateContact(props.id)">
              ...
              <div class="card-actions w-full">
                <button type="submit" class="btn btn-success w-full text-white">Update</button>
                <div class="divider w-full">OR</div>
                <button type="button" class="btn btn-error w-full text-center text-white" @click="onOpenModal">Delete</button>
              </div>
            </form>
          </div>
        </div>
    ...
    <dialog id="my_modal_5" class="modal modal-bottom sm:modal-middle">
      <div class="modal-box">
        ...
        <div class="modal-action">
          <form method="dialog" class="space-x-2" @submit="store.deleteContact(props.id)">
            <button class="btn w-36">Close</button>
            <button type="submit" class="btn btn-error text-white w-36"> Yes </button>
          </form>
        </div>
      </div>
    </dialog>
